...
import static org.deepinthink.magoko.boot.msocket.MSocketConstants.DEFAULT_MSOCKET_SERVER_DAEMON_THREAD_NAME;
...
final class NettyMSocketServer implements MSocketServer {
  ...
  private void startDaemonAwaitThread(DisposableServer server) {
    Thread awaitThread =
        new Thread(() -> server.onDispose().block(), DEFAULT_MSOCKET_SERVER_DAEMON_THREAD_NAME);
